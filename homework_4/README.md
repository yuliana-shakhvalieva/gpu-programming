...
1. Реализуйте расчет фрактала Мандельброта на OpenCL - см. ```src/main_mandelbrot.cpp``` и ```src/cl/mandelbrot.cl```.

#### Задание 4.2. Суммирование чисел

1. Реализуйте суммирование чисел на OpenCL - см. ```src/main_sum.cpp```.

2. Реализуйте несколько методов:

- Суммирование с глобальным атомарным добавлением (просто как бейзлайн)

- Суммирование с циклом

- Суммирование с циклом и coalesced доступом (интересно сравнение по скорости с не-coalesced версией)

- Суммирование с локальной памятью и главным потоком

- Суммирование с деревом

3. Посмотрите на результаты производительности и попробуйте их проанализировать с учетом своего железа, обсуждения на лекции и ожиданий, какая версия должна была бы стать самой быстрой. 
